/* Generated by re2c 0.13.5 on Mon Feb  1 16:08:06 2016 */
#line 1 "Config.r2c"
/*
		...
		if (mode() == MODE_INCREMENTAL || mode() == MODE_REACTIVE) {
			...
			if (subconfig) {
				if (subconfig->minstep != UNDEFINED) {
					// arglist.push_back("--imin=" + utils::to_string(subconfig->minstep));
					str += "#const imin=" + utils::to_string(subconfig->minstep) + ".\n";
				}
				if (subconfig->maxstep != UNDEFINED) {
					if (subconfig->maxstep > 0){

						// arglist.push_back("--imax=" + utils::to_string(subconfig->maxstep));
						str += "#const imax=" + utils::to_string(subconfig->maxstep) + ".\n";
					}else{
				...
				if (intConfigOpt(OPT_MINSTEP) != UNDEFINED) {
					// arglist.push_back("--imin=" + utils::to_string(			intConfigOpt(OPT_MINSTEP)));
					str += "#const imin=" + utils::to_string(subconfig->minstep) + ".\n";
				}
				if (intConfigOpt(OPT_MAXSTEP) != UNDEFINED){
					if (intConfigOpt(OPT_MAXSTEP) > 0){	// arglist.push_back("--imax=" + utils::to_string(intConfigOpt(OPT_MAXSTEP)));
						str += "#const imax=" + utils::to_string(subconfig->maxstep) + ".\n";
					}else{
			...
			// Curently trying option b. the output file is located at output(TC_C4_PREPROC).

			std::ofstream out;
			out.open(output(TC_C4_PREPROC), std::ios::app);
			out << str;
			out << incremental::python_code;
