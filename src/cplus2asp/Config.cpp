/* Generated by re2c 0.13.5 on Thu Mar 17 20:41:31 2016 */
#line 1 "Config.r2c"
/*
...
#ifdef DEBUG
static const char * ToolChainStrings[] = {"None","translator.op","f2lp.op","c3to4.op","c4.op","as2.op","reb.op","End","Length"};
#endif
...
#ifdef DEBUG
const char * Config::getTextForEnum( int enumVal )
{
  return ToolChainStrings[enumVal];
}
#endif
...
void Config::compileArgs(Toolchain tool, std::list<std::string>& arglist, RunConfig const* subconfig)  const {
	...
	case TC_SOLVER:
		...
		if (subconfig) {
			if (subconfig->query != "") arglist.push_back("-c queryl=q_" +subconfig->query);
			...
			if (strOpt(STR_QUERY) != "") arglist.push_back("-c queryl=q_" + utils::to_string(strOpt(STR_QUERY)));
		...
		if (!run(TC_C4_PREPROC)) {
			if (subconfig) {
				...
				if (strOpt(STR_QUERY) != "") arglist.push_back("-c queryl=q_" + strOpt(STR_QUERY));
				if (intConfigOpt(OPT_MAXSTEP) != UNDEFINED) arglist.push_back("-c maxstep=" + utils::to_string(intConfigOpt(OPT_MAXSTEP)));
				if (intConfigOpt(OPT_MINSTEP) != UNDEFINED) arglist.push_back("-c minstep=" + utils::to_string(intConfigOpt(OPT_MINSTEP)));
			}
		...
		if (mode() == MODE_INCREMENTAL || mode() == MODE_REACTIVE) {
			if (subconfig) {
				if (subconfig->minstep != UNDEFINED) {
					arglist.push_back("-c imin=" + utils::to_string(subconfig->minstep+1));
				}
				if (subconfig->maxstep != UNDEFINED) {
					if (subconfig->maxstep > 0){
						arglist.push_back("-c imax=" + utils::to_string(subconfig->maxstep+1));
					}else{
				...
				if (intConfigOpt(OPT_MINSTEP) != UNDEFINED) {
					arglist.push_back("-c imin=" + utils::to_string(intConfigOpt(OPT_MINSTEP)));
				}
				if (intConfigOpt(OPT_MAXSTEP) != UNDEFINED){
					if (intConfigOpt(OPT_MAXSTEP) > 0){	
						arglist.push_back("-c imax=" + utils::to_string(intConfigOpt(OPT_MAXSTEP)+1));
					}else{
			...
			// Clingo4 requires a python file as input along with the clingo file to be processes to make
			// it work in the incremental mode. 
			// There are 2 options 
			// a) include a file with the python script 
			// b) append python code to that file
			// c) include incmode header in file. Its lua script for incremental encoding embedded in clingo if compiled with lua support.
			// Curently trying option c. the output file is located at output(TC_C4_PREPROC).
			if(!subconfig->is_pyscript_added){
				std::ofstream out;
				out.open(output(TC_C4_PREPROC), std::ios::app);
				out << "\n#include <incmode>.\n";
				subconfig->is_pyscript_added = true;
			}
